...
class PlayState extends MusicBeatState
{
	...
	var sky:BGSprite;
	var cloudback:BGSprite;
	var sun:BGSprite;
	var cloudfront:BGSprite;
	var cloudmain:BGSprite;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			case 'dasky': //Week Roboscrew
			    var sky:BGSprite = new BGSprite('sky', -600, -200, 0.2, 0.2);
				add(sky);

				var cloudback:BGSprite = new BGSprite('cloud-back', -600, -200, 0.5, 0.5);
				add(cloudback);

				var sun:BGSprite = new BGSprite('sun', -600, -200, 0.6, 0.6);
				add(sun);

				var cloudfront:BGSprite = new BGSprite('clouds-front', -600, -200, 0.7, 0.7);
				add(cloudfront);

				var cloudmain:BGSprite = new BGSprite('clouds-chars', -600, -200);
				add(cloudmain);

			case 'placeholder': //Neon is a poopyhead
				var bg:BGSprite = new BGSprite('placeholder-stage', -600, -800, 0.9, 0.9);
				add(bg);

			case 'stage': //Default Stage if another is not found or smth
		...
		// Watermarks, this is for Vs Roboscrew
		rbWatermark = new FlxText(0, FlxG.height - 710, 0, "VS Roboscrew: v" + MainMenuState.roboscrewVersion, 16);
		rbWatermark.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		rbWatermark.borderSize = 2;
		rbWatermark.borderQuality = 2;
		rbWatermark.scrollFactor.set();
		rbWatermark.updateHitbox();
		rbWatermark.x = FlxG.width - rbWatermark.width - 5;
		rbWatermark.visible = false;
		...
		// Song Display thingy
		songDisplay = new FlxText(0, FlxG.height - 670, 0, SONG.song + " - " + CoolUtil.difficultyString(), 16);
  ...
  public function clearShaderFromCamera(cam:String){
	...
	override function beatHit()
	{
		...
		if(curBeat % 1 == 0) {
			...
			iconP1.angle = 15;
			FlxTween.angle(iconP1, 15, 0, 0.2, {ease: FlxEase.Out, type: PINGPONG});
			iconP2.angle = 15;
			FlxTween.angle(iconP2, 15, 0, 0.2, {ease: FlxEase.Out, type: PINGPONG});
		}
