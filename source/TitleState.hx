...
class TitleState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		if(!closedState) {
			...
			var http = new haxe.Http("https://fnf-versions.crushernotdrip.repl.co/roboscrew.txt");
			
			http.onData = function (data:String)
			{
				updateVersion = data.split('\n')[0].trim();
				var curVersion:String = MainMenuState.roboscrewVersion.trim();
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			{
				case 1:
					createCoolText(['A project by'], 15);
